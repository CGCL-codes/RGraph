#ifndef GRAPHLAB_DISTRIBUTED_RDMA_INGRESS_HPP
#define GRAPHLAB_DISTRIBUTED_RDMA_INGRESS_HPP
...
namespace graphlab {
   ...
   */
  template<typename VertexData, typename EdgeData>
  class distributed_rdma_ingress:
  ...
  public:
    distributed_rdma_ingress(distributed_control& dc, graph_type& graph, bool usehash = false, bool userecent = false) :
      base_type(dc, graph),
    ...
    ~distributed_rdma_ingress() { }

    /** Add an edge to the ingress object using rdma greedy assignment. */
  ...
  }; // end of distributed_rdma_ingress
