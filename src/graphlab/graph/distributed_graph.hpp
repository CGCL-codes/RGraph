...
#include <graphlab/graph/ingress/distributed_rdma_ingress.hpp>
...
namespace graphlab {
  ...
  public:
    ...
    friend class distributed_rdma_ingress<VertexData, EdgeData>;
  ...
  private:
    ...
    void set_ingress_method(const std::string& method,
      ...
      if (method == "oblivious") {
        ...
        if (rpc.procid() == 0) logstream(LOG_EMPH) << "Use rdma ingress, usehash: " << usehash
          << ", userecent: " << userecent << std::endl;
        ingress_ptr = new distributed_rdma_ingress<VertexData, EdgeData>(rpc.dc(), *this, usehash, userecent);
      } else if (method == "hdrf") {
