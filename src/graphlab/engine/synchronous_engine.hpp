...
//      timeval gather_start, gather_end;
//      gettimeofday(&gather_start, NULL);
      run_synchronous( &synchronous_engine::execute_gathers );
//      gettimeofday(&gather_end, NULL);
//      double delta_t1 = (gather_end.tv_sec - gather_start.tv_sec) * 1000 +  (gather_end.tv_usec - gather_start.tv_usec) / 1000;
//      communication_time += delta_t1;
...
//      timeval apply_start, apply_end;
//      gettimeofday(&apply_start, NULL);
      run_synchronous( &synchronous_engine::execute_applys );
//      gettimeofday(&apply_end, NULL);
//      double delta_t = (apply_end.tv_sec - apply_start.tv_sec) * 1000 +  (apply_end.tv_usec - apply_start.tv_usec) / 1000;
//      total_apply += delta_t;
      /**
       * Post conditions:
...
//      timeval scatter_start, scatter_end;
//      gettimeofday(&scatter_start, NULL);
      run_synchronous( &synchronous_engine::execute_scatters );
//      gettimeofday(&scatter_end, NULL);
//      double delta_t2 = (scatter_end.tv_sec - scatter_start.tv_sec) * 1000 +  (scatter_end.tv_usec - scatter_start.tv_usec) / 1000;
//      communication_time += delta_t2;
      /**
       * Post conditions:
       *   1) NONE
       */
...
//    double total_time = (total_end.tv_sec - total_start.tv_sec) * 1000 +  (total_end.tv_usec - total_start.tv_usec) / 1000;
//
//    logstream(LOG_INFO) << "Computation time is " << total_apply << "ms" << std::endl;
//    logstream(LOG_INFO) << "Communication time is " << communication_time << "ms" << std::endl;
//    logstream(LOG_INFO) << "Total time is " << total_time << "ms" << std::endl;
